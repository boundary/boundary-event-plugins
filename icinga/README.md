Icinga Boundary Events Handler
===

This script is a fork of the Nagios adapter for use with Icinga to send host and service status changes to the Boundary Event service.
...
Add the config file `boundary.yml` to `/etc/icinga/boundary.yml` (If you choose a different directory, you'll need to update the `BOUNDARY_CONFIG_PATH` variable in the script.) and ensure it has the following settings:

- sender: <the fqdn of your Icinga server as it would appear in Boundary>
...
Add the following commands to your Icinga instance:
...
Add the following to your `Icinga.cfg` (the first three lines may already exist):
